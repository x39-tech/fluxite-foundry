...
Everything under `/src` runs on both targets. Platform differences are typically resolved at runtime. Code needing a native capability branches on `isTauri()` from `@tauri-apps/api/core` and keeps a browser fallback.

Tauri plugins are imported lazily inside the desktop branch, so the web bundle never pulls them in.

### Versioning

`package.json` holds the version and is the single source of truth. `tauri.conf.json` reads it via `"version": "../package.json"`, and Vite injects the same value as `__APP_VERSION__` so it's readable from the app. `src-tauri/Cargo.toml` carries no version field and doesn't need one.

The web app deploys from every commit to `main` and is identified by `__BUILD_STRING__` (timestamp, commit hash, and a random word). Desktop releases are cut deliberately from a tag and assigned a version number.
...
├── capabilities/                   # Tauri capabilities files governing permissions for the webview
├── *.conf.json                     # Tauri config files for various platforms
...
The application uses [FlexLayout React](https://github.com/nicerobot/FlexLayout) for a docking/tabbed interface. Layout state is saved per document as JSON in `session.layouts`, which is persisted locally but deliberately does not end up in save files.
