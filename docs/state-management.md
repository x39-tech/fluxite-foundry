...
A _save file_ holds one document and has the extension `.ffd`. It is a zip containing:

```
document.json     # { formatVersion, stateVersion, documentType, document, assets }
assets/<assetId>  # the bytes of each asset the document refers to
```

At the time of writing, Fluxite Foundry save files are pretty similar to Fluxite Codex Archives in what they contain; the key differences are:

1. A Fluxite Foundry save file always represents exactly one document (device class, library or system)
2. As we add richer functionality to documents, such as user notes and progress tracking, these features will land in the save file and provide value over the export format.

`formatVersion` versions the overall save file container and thus we make a format commitment now: a save file is always a zip with a file called `document.json` at the root which has at least one key `formatVersion`. `stateVersion` is the persistent state version the document inside is written in; that drives version migration of the document.

This lives in `app/documentFile.ts`, and the platform-specific halves of asking the user for a file are in `app/openFile.ts` and `app/saveFile.ts`.

Note that save files are not the same as state snapshots (see below); state snapshots are more of a debugging feature and capture the entire app's persistent state as opposed to one document.
...
│   ├── migrate.test.ts
│   ├── envelope.json   # Generated, loaded by the app
│   └── snapshot.json   # Generated, test fixture
└── ...
```
...
- `envelope.json` - Generated by `npm run state-version:capture`, and used to migrate save files. See "Envelopes" below.
- `snapshot.json` - Generated by the same command, and used for migration integration testing. See "The Snapshot History" below.
...
The chain migrates a whole `AppPersistentState`, but a save file holds a single document. So to open one written at an older version, the document is wrapped in the smallest valid state of its own version, migrated, and taken back out. We call the wrapper an _envelope_ and maintain one for each historic state version. See `app/documentFileEnvelopes.ts`.
...
8. **Capture the new version's snapshot and envelope.** Run `npm run state-version:capture` and commit the `snapshot.json` and `envelope.json` it writes into `src/app/persistentState/vN/`.
...
Each version directory holds a `snapshot.json`, so between them they cover every version the app has ever written state at. `snapshotHistory.test.ts` loads each one, migrates it to the current version, and asserts the result validates. The same test fails if any version has no snapshot, the current one included.

The snapshots are written by `npm run state-version:capture`, which builds one by importing the example device classes shipped with the E1.73 spec through the app's own import path. This isn't perfect coverage because it only tests data that is imported from Fluxite Codex, but combined with migration unit tests, it makes a pretty decent harness.
